...
gates Docker/cpu-basic Spaces behind PRO; ZeroGPU remains free). The REST
routes are attached to gradio's own FastAPI server at launch (the ZeroGPU
harness owns the port until `launch()` runs); the Gradio UI serves at `/`.
**All inference runs on CPU** — Kronos-small is 24.7M params — so requests
consume **no GPU quota**; the UI's "ZeroGPU smoke test" button is the only
GPU-decorated function.
...
| `/` | 〃 | human status page (Gradio) + ZeroGPU smoke test |
