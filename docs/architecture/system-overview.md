...
recommendations, and serves a Next.js dashboard + grounded chat.

## Components

| Component | Tech | Responsibility |
|---|---|---|
| Frontend | Next.js 15 (App Router, TS), Tailwind v4, TanStack Query, lightweight-charts | Dashboard, instrument detail (candles + forecast overlay + backtest), agent transcripts, chat; auth UI; same-origin authenticated proxy to the backend |
| Backend | FastAPI (async), SQLAlchemy 2 + asyncpg, APScheduler | REST APIs, auth/RBAC, forecasting/backtesting orchestration, agent pipeline, RAG chat, scheduler |
...
durable job queue (Phase 5); multi-tenant RLS policies (all data access is
